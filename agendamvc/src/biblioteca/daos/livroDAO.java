...
public class livroDAO {
	...
	public boolean inserir(Livro livro) {
		...
		try {
			...
			stmt.setDate(5, new java.sql.Date(livro.getAnoPublicado().getTimeInMillis()));
	...
	public List<Livro> getLista() {
		...
		try {
			...
			while (rs.next()) {
				...
				data.setTime(rs.getDate("AnoPublicado"));
	...
	public boolean alterar(Livro livro) {
		String sql = "update livro set titulo=?, autor=?, editora=?, edicao=?, anoPublicado=? where id=?;";
		try {
			PreparedStatement stmt = connection.prepareStatement(sql);
			...
			stmt.setString(2, livro.getAutor());
			stmt.setString(3, livro.getEditora());
			stmt.setDate(4, new java.sql.Date(livro.getAnoPublicado().getTimeInMillis()));
